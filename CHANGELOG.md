...
* **The window chrome scales, independently of the editor font.** View → Font gains **Bigger Interface** (⌃⌘=), **Smaller Interface** (⌃⌘−) and **Default Interface Size** (⌃⌘0), in steps of 0.1 from 0.8 to 3.0, applied live and kept across launches. Tab bar, document and HTML output status bars, file browser (header, action bar, rows, icons, glyphs), Find, the choosers (Jump to File, Jump to Symbol, Select Bundle Item, Open Recent Project, Clipboard History), Filter Through Command, Unknown Encoding, Commit, Software Update, the first-launch bundle installer, Jump to Line, Tab Size, Customize Available Encodings, the pasteboard selector, Preferences, tool tips, the zoom HUD, the status bar and file browser menus and command output in the HTML window all follow. Requested by [@GregDunn1953](https://github.com/GregDunn1953) for a 4K display. ([#63](https://github.com/textmatelives/textmate/issues/63), [#71](https://github.com/textmatelives/textmate/pull/71), [#72](https://github.com/textmatelives/textmate/pull/72), [#73](https://github.com/textmatelives/textmate/pull/73), [#74](https://github.com/textmatelives/textmate/pull/74), [#75](https://github.com/textmatelives/textmate/pull/75), [#76](https://github.com/textmatelives/textmate/pull/76), `08ce6f36`, `82992439`, `93f6375a`, `dc58dc85`, `14dfacae`, `72f1d39b`)
* **What does not scale.** The menu bar, alerts, the Open and Save panels, context menus, the standard control tool tips, the Preferences toolbar and the Bundle Editor are drawn by macOS at the system size. The completion popup follows the editor font on purpose.
* **The scale is also a default.** `defaults write com.macromates.TextMate uiFontScaleFactor 1.5` (a bare number is fine; the key is removed at 1). The existing `statusBarFontSize`, `OakBundleManagerDisambiguateMenuFontSize`, `tabItemMinWidth` and `tabItemMaxWidth` keys now give the base size that the scale multiplies; `searchResultsFontName`/`Size`, `lineNumberFontName` and `lineNumberScaleFactor` are unchanged. All of them are listed in Help → TextMate Help under “Interface Scale”. (`7cd18b5f`, `10844a62`, `84634bba`)
* **Standard controls are zoomed, not re-fonted.** A bezelled button or checkbox takes its height from its control size, not its font, so re-fonting a dialog breaks it. Dialogs are instead placed in `OakScaledContainerView`, which zooms a whole content view and resizes the window by the ratio about its top-left, capped so the window still fits the screen. Windows restored from a frame saved at another scale come back at the right size. ([#72](https://github.com/textmatelives/textmate/pull/72), `82992439`)

### Software Update

* **The update dialog shows the release notes before you download.** `release.yml` renders the version’s section of this file to `TextMate-{version}-notes.html` next to the archive; the app fetches it by convention only when a newer version is offered (no Releases API, per [#26](https://github.com/textmatelives/textmate/issues/26)) and renders it with JavaScript off under a Content-Security-Policy that permits nothing but its own stylesheet. Links open in the browser. Releases without the asset show the dialog as before. Suggested by [@damienmckenna](https://github.com/damienmckenna). ([#56](https://github.com/textmatelives/textmate/issues/56), [#67](https://github.com/textmatelives/textmate/pull/67), `00dd741a`)
* **After an update, TextMate offers the release notes instead of opening them.** A small notice names the new version, with **Changelog** as the default button and **Cancel** beside it. This had been dead since 2021: `showChangesIfUpdated` looked the notes up without the `About/` subdirectory, so the URL was `nil`. The trigger is now the running version against a stored `lastLaunchedVersion`. ([#56](https://github.com/textmatelives/textmate/issues/56), [#59](https://github.com/textmatelives/textmate/pull/59), `95101c7e`)
* **A third update channel, “Experimental builds”, carries feature streams that are not on the mainline.** All three channels read the one `textmatelives/textmate` feed; the streams are separated by the tag marker `release.yml` stamps — none, `-beta`, `-exp`. Release sees stable tags only. Beta sees stable and `-beta`, so a beta user converges back onto stable once it catches up. Experimental sees `-exp` and deliberately **not** stable: such a stream may never merge, and offering a newer stable would silently take away the feature the user opted in to test. ([#58](https://github.com/textmatelives/textmate/pull/58), `79070871`)
* **Fixed: a non-beta prerelease tag would have been offered to everyone.** The version picker skipped only tags containing `-beta`, so a `-exp` tag would have outranked the stable release on the stable channel. Admission is now decided per channel, and an unrecognised or missing channel resolves to release. ([#58](https://github.com/textmatelives/textmate/pull/58), `79070871`)
* **The “nightly” channel is gone, along with “Check for Test Build”.** It saw exactly what Release saw, and its only entry point, the ⌥-key alternate menu item, duplicated “Check for Update”. The channel is now read solely from Preferences → Software Update. An empty channel is reported as such rather than as a server error. ([#58](https://github.com/textmatelives/textmate/pull/58), `79070871`)

### Editor

* **⌘Q works while the Commit sheet is up.** A sheet prevents application termination by default, and AppKit refused before asking the app, so quitting appeared to do nothing for as long as the commit tool waited. Quitting now abandons the commit, as cancelling it would. ([#74](https://github.com/textmatelives/textmate/pull/74), `dc58dc85`)
* **The file browser’s SCM status button names its shortcut** (⇧⌘Y) in its tool tip; the menu item that owns it sits under File Browser, which is not where one looks for an SCM action. ([#61](https://github.com/textmatelives/textmate/pull/61), `8b6c153d`)

### Bundles

* **⌘/ no longer raises on lines with multibyte characters.** `TM_LINE_INDEX` is a UTF-8 byte offset and Ruby 2’s `String#[]` indexes characters, so with the caret past an accent or a symbol, Toggle Comment either raised `IndexError` or put the caret in the wrong column. A regression from dropping the ruby18 shebangs, whose `String#[]` was byte-indexed. Source pin `678a8db1` → `24eb8e60`; mandatory bundles ship inside the app, so the pin is the delivery. Reported and fixed by [@tbates](https://github.com/tbates). ([#66](https://github.com/textmatelives/textmate/pull/66), [source.tmbundle#2](https://github.com/textmatelives/source.tmbundle/pull/2), `c0806a32`)

### Build

* **Warning sweep: 503 → ~177 lines in a clean Debug build.** `std::iterator` base classes replaced with member typedefs (242 lines); vendor deprecations, duplicate-library and no-symbols noise silenced where no change to TextMate’s code could remove them (62); and 22 deprecated API sites retired where the replacement is provably identical — same declared value, or the same rendered pixels, checked by rasterising each icon pair. Six `iconForFileType:` sites stay because no UTType reproduces their icon. ([#65](https://github.com/textmatelives/textmate/issues/65), [#68](https://github.com/textmatelives/textmate/pull/68), [#69](https://github.com/textmatelives/textmate/pull/69), [#70](https://github.com/textmatelives/textmate/pull/70), `f9801692`, `2f6bcbee`, `6b058fc5`)
* **Debug builds no longer abort when opening an XML property list.** The first line of a file was handed to the regex engine in a buffer of exactly its size, and the property list grammar’s first-line pattern reads one byte past it. Release builds only ever read a stray byte. ([#75](https://github.com/textmatelives/textmate/pull/75), `b5923e86`)
* **Removed a development bundle identifier that never took effect.** The CMake migration carried a `BUNDLE_IDENTIFIER` meant to give non-Release builds a `com.macromates.TextMate-dev` identity, but `Info.plist` never read it. Removed rather than completed: `mate` looks the app up by the stable identifier, so a separate development identity would point it at the wrong build. ([#60](https://github.com/textmatelives/textmate/pull/60), `38a4810d`)

### Documentation

* **`RELEASING.md` no longer claims every release becomes `releases/latest`;** `-beta` and `-exp` versions publish as GitHub prereleases. ([#62](https://github.com/textmatelives/textmate/pull/62), `9b97009d`)
